...
def view_post(request, id):
    ...
    return render(request, 'post/view_post.html', {'post':post, 'comment':comment})
...
def sign_up(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            f = Profile(name='',email=u.email,contact=+91,dob=datetime.now(),gender='',pic='/static/images/user.png', user=u)
            f.save()
...
def profile(request):
    ...
    return render(request, 'user/profile.html', {'post': post})
